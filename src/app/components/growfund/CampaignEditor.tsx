...
export default function CampaignEditor({ id }: { id: string }) {
  ...
  const [section, setSection] = useState<"basics" | "goal" | "confirmation">("basics");
    ...
    <CardBox>
      <div className="mb-6 grid grid-cols-3 gap-2">
        {[
          ["basics", "Basics"],
          ["goal", "Goal"],
          ["confirmation", "Donation Confirmation Message"],
        ].map(([key, title]) => (
          <button key={key} type="button" onClick={() => setSection(key as any)} className={`rounded-xl px-3 py-3 text-center text-xs font-bold sm:text-sm ${section === key ? "bg-primary text-white" : "bg-lightgray text-darklink"}`}>{title}</button>
        ))}
      </div>
      <form onSubmit={submit} className="grid grid-cols-1 gap-5 md:grid-cols-2">
        {section === "basics" && <>
          ...
          <SelectField label="Category" value={form.category} onChange={(value: string) => { set("category", value); set("sub_category", ""); }} error={fieldErrors.category}>
            <option value="">Select category</option>{parents.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}
          </SelectField>
          <SelectField label="Sub-category" value={form.sub_category} onChange={(value: string) => set("sub_category", value)} error={fieldErrors.sub_category} disabled={!form.category || children.length === 0}>
            <option value="">No sub-category</option>{children.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}
          </SelectField>
          <SelectField label="Location" value={form.location} onChange={(value: string) => set("location", value)} error={fieldErrors.location} className="md:col-span-2">
            <option value="">Select location</option>{locations.map((location) => <option key={location.value} value={location.value}>{location.label}</option>)}
          </SelectField>
        </>}
        {section === "goal" && <>
        ...
        {section === "confirmation" && <>
          <Field label="Title" value={form.confirmation_title} onChange={(value: string) => set("confirmation_title", value)} className="md:col-span-2" />
          <TextArea label="Description" value={form.confirmation_description} onChange={(value: string) => set("confirmation_description", value)} className="md:col-span-2" rows={5} />
          <div className="md:col-span-2">
            <div className="mb-3 flex items-center justify-between"><h6 className="font-semibold text-dark">Frequently Asked Questions</h6><Button type="button" variant="outline" onClick={() => setFaqs((current) => [...current, { question: "", answer: "" }])}><Icon icon="solar:add-circle-line-duotone" /> Add FAQ</Button></div>
            <div className="space-y-4">{faqs.map((faq, index) => <div key={index} className="rounded-xl border border-ld p-4">
              <Field label="Question" value={faq.question} onChange={(value: string) => setFaqs((current) => current.map((item, i) => i === index ? { ...item, question: value } : item))} />
              <div className="mt-3"><TextArea label="Answer" value={faq.answer} onChange={(value: string) => setFaqs((current) => current.map((item, i) => i === index ? { ...item, answer: value } : item))} rows={4} /></div>
              <button type="button" className="mt-3 text-sm font-medium text-error" onClick={() => setFaqs((current) => current.filter((_, i) => i !== index))}>Remove FAQ</button>
            </div>)}</div>
          </div>
        </>}
