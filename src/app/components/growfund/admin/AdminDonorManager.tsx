...
export default function AdminDonorManager() {
  ...
  const load = useCallback(async () => { setLoading(true); setError(""); try { const q = new URLSearchParams({ page: "1", per_page: "100" }); if (status !== "all") q.set("status", status); if (search.trim()) q.set("search", search.trim()); const range=dateRangeParams(dateRange); q.set("start_date",range.start_date); q.set("end_date",range.end_date); setRows(rowsFrom(await adminApi(`donors/paginated?${q}`))); } catch (e) { setError(e instanceof Error ? e.message : "Unable to load donors."); } finally { setLoading(false); } }, [status, search, dateRange]);
  ...
  return <CardBox>
    ...
    <div className="mt-4 overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Donor Details</TableHead><TableHead>Donations</TableHead><TableHead>Total Given</TableHead><TableHead>Latest Donation</TableHead><TableHead>Date Created</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>
      {loading ? <TableRow><TableCell colSpan={6} className="py-10 text-center">Loading donors…</TableCell></TableRow> : rows.length === 0 ? <TableRow><TableCell colSpan={6} className="py-10 text-center text-darklink">No donors found.</TableCell></TableRow> : rows.map((r) => { const id = idOf(r), st = String(r.status || r.user_status || "active").toLowerCase(); const latest = latestDonation(r); return <TableRow key={id}>
