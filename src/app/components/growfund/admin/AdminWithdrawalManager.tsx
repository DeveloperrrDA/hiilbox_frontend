...
export default function AdminWithdrawalManager() {
  ...
  const visibleRows = useMemo(() => rows.filter((row) => {
    ...
    if (startDate && (!parsedDate || parsedDate < new Date(`${startDate}T00:00:00`))) return false;
    if (endDate && (!parsedDate || parsedDate > new Date(`${endDate}T23:59:59`))) return false;
    if (!isDateInRange(rawDate, dateRange)) return false;
    return true;
  }), [rows, search, status, dateRange, startDate, endDate]);
  const totalPages = Math.max(1, Math.ceil(visibleRows.length / 10));
  const pageRows = visibleRows.slice((page - 1) * 10, page * 10);
  useEffect(() => setPage(1), [search, status, dateRange, startDate, endDate]);
