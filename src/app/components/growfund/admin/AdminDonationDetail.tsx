...
export default function AdminDonationDetail({ donationId }: Props) {
  ...
  const emailVerified = emailVerifiedRaw === true || emailVerifiedRaw === 1 || emailVerifiedRaw === "1" || Boolean(typeof emailVerifiedRaw === "string" && emailVerifiedRaw && emailVerifiedRaw !== "false");
  const billingAddress = deepValue(donation, ["billing_address", "address"]);
  const paymentMethod = paymentMethodLabel(
    donation?.transaction?.payment_method ??
    donation?.transaction?.gateway ??
    donation?.payment?.method ??
    donation?.payment?.payment_method ??
    donation?.payment_engine ??
    donation?.gateway ??
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <CardBox>
              <div className="flex gap-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-sm text-darklink">by {fundraiserName} · {campaignStartDate ? fmtDate(campaignStartDate) : "Start date unavailable"}</p>
          ...
          <div className="space-y-5">
            <CardBox>
              ...
              <div className="mt-5 space-y-4 text-sm"><div className="flex flex-wrap items-center gap-3"><Icon icon="solar:letter-line-duotone" className="text-xl text-darklink" /><span className="break-all">{donorEmail}</span><Badge variant={emailVerified ? "lightSuccess" : "lightWarning"}>{emailVerified ? "verified" : "awaiting verification"}</Badge></div>{billingAddress && <div className="flex gap-3"><Icon icon="solar:map-point-line-duotone" className="mt-0.5 text-xl text-darklink" /><span>{typeof billingAddress === "string" ? billingAddress : Object.values(billingAddress).filter(Boolean).join(", ")}</span></div>}</div>
            </CardBox>
