...
export default function FundraiserWallet() {
  ...
  async function requestWithdrawal(
    ...
    try {
      ...
      // Refresh the page so the withdrawal
      // history also reloads immediately.
  ...
  return (
    <div className="space-y-7">
      {error && (
        <div className="rounded-md border border-error/30 bg-lighterror px-4 py-3 text-sm text-error">
          {error}
        </div>
      )}

      {notice && (
        <div className="rounded-md border border-success/30 bg-lightsuccess px-4 py-3 text-sm text-success">
          {notice}
        </div>
      )}
      ...
      <CardBox>
        ...
        <div className="mt-7 grid rounded-md border border-ld sm:grid-cols-3">
          {[
            ["Net Balance", netBalance],
            ["Pending Payouts", pending],
            ["Total Withdrawal", withdrawn],
          ].map(([label, value], index) => (
            <div
              key={String(label)}
              className={`p-5 ${
                index
                  ? "border-t border-ld sm:border-l sm:border-t-0"
                  : ""
              }`}
            >
      ...
      >
        <DialogContent className="sm:max-w-xl">
          ...
          <div className="rounded-md bg-lightgray p-6">
            <p className="text-darklink">
              Available to withdraw
            </p>
